<template>
  ...
  <div v-else class="grid grid-cols-6 gap-x-8 gap-y-4">No Data Found!!!</div>
</template>
...
const baseurl =
  ctype == "true"
    ? "http://localhost:8020/training/"
    : "http://localhost:8020/training/";
const fetchImages = async (): Promise<void> => {
  try {
    const response = await fetch(
      `http://localhost:8020/training-frames-list/${queryParam}?type=${ctype?.toString()}`,
      {
        method: "GET",
      }
    );
